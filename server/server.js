...
app.get("/ga/races", async (req, res) => {
  try {
    const axiosResponse = await axios.get(`${API_GA}/races`);
...
app.get("/ga/results", async (req, res) => {
  try {
    const axiosResponse = await axios.get(`${API_GA}/results`);
    console.log(axiosResponse);
    res.json(axiosResponse.data);
  } catch (error) {
    res.status(500).send(error.message);
...
app.get("/ga/members/:id/results", async (req, res) => {
  try {
    const axiosResponse = await axios.get(
      `${API_GA}/members/${req.params.id}/results`
    ); // endret variabelnavnet til axiosResponse
    res.json(axiosResponse.data);
  } catch (error) {
    res.status(500).send(error.message);
  }
